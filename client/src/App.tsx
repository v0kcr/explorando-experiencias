...
function DemoForm({ button, kind, corporate = false, ebook = false, contact = false }: { button: string; kind: "trip" | "corporate" | "club" | "ebook" | "contact"; corporate?: boolean; ebook?: boolean; contact?: boolean }) {
  const [sent, setSent] = useState(false);
  const createInquiry = trpc.inquiries.create.useMutation({ onSuccess: () => setSent(true) });
  const submit = (event: FormEvent<HTMLFormElement>) => {
    ...
    createInquiry.mutate({
      ...
      destination: String(values.get("destination") || "") || undefined,
      message: String(values.get("message") || "") || undefined,
    });
  };
  if (sent) return <div className="success-state"><div><Check size={22} /></div><h3>¡Listo! Recibimos tus datos.</h3><p>Gracias por escribirnos. Nuestro equipo revisará tu solicitud y te contactará pronto.</p><button onClick={() => setSent(false)}>Enviar otra respuesta</button></div>;
  return <form className="demo-form" onSubmit={submit}>
    ...
    {!ebook && <label>{corporate ? "Empresa" : "Teléfono"}<input name={corporate ? "company" : "phone"} required placeholder={corporate ? "Nombre de tu empresa" : "+51 999 999 999"} /></label>}
    <label>Email<input name="email" type="email" required placeholder="tu@email.com" /></label>
    {corporate && <label>Cuéntanos un poco más<textarea name="message" required placeholder="Número de personas, fecha tentativa, objetivo..." rows={3} /></label>}
    {kind === "trip" && <label>¿Qué destino te interesa?<select name="destination" defaultValue="" required><option value="" disabled>Selecciona una opción</option><option>Cusco & Valle Sagrado</option><option>Paracas & Huacachina</option><option>Huaraz: Laguna 69</option><option>Aún no lo sé</option></select></label>}
    {contact && <label>Cuéntanos tu idea<textarea name="message" required placeholder="¿En qué podemos ayudarte?" rows={3} /></label>}
    <button className="button primary" type="submit" disabled={createInquiry.isPending}>{createInquiry.isPending ? "Guardando..." : button} {!createInquiry.isPending && <ArrowRight size={16} />}</button>
    {createInquiry.isError && <small className="form-error">No pudimos guardar tu solicitud. Intenta nuevamente.</small>}
    <small>Al enviar aceptas nuestra <Link href="/privacy-policy">política de privacidad</Link>.</small>
  </form>;
}
